...
export default function ChatPage() {
  ...
  // Load messages for selected conversation — refresh every 5s
  ...
  return (
    <div className="flex h-full -m-6 bg-gray-100">
      ...
      <div className="w-16 bg-[#1a1a2e] flex flex-col items-center py-3 gap-2 shrink-0">
          ...
          return (
            <button
              ...
              className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold transition-all ${
                isActive ? "bg-brand-500 text-white shadow-lg scale-105" : "bg-white/10 text-white/70 hover:bg-white/20"
              }`}
            >
              {initials}
            </button>
        ...
        {accounts.length === 0 && (
          <div className="text-white/30 text-xs text-center px-1 mt-4">No phones</div>
        )}
      </div>
        ...
        <div className="px-4 py-3 border-b border-gray-100 bg-[#f0f2f5]">
          <div className="flex items-center justify-between mb-2">
            <span className="font-semibold text-gray-800 text-sm">
              {accounts.find((a) => a.id === selectedCredId)?.name ?? "Conversations"}
            </span>
            ...
            <div className="px-4 py-3 bg-[#f0f2f5] border-b border-gray-200 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500">
                  via {activeConv?.waCredential.name ?? activeConv?.waCredential.displayPhone}
                </div>
              </div>
            </div>
                ...
                return (
                  <div key={msg.id} className={`flex ${isOut ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[65%] ${isOut ? "items-end" : "items-start"} flex flex-col`}>
                      <div className={`px-3 py-2 rounded-lg text-sm shadow-sm relative ${
