    ...
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${TIER_COLORS[sub.tier]}`}>
                {sub.tier}
              </span>
      ...
      {result && (
        <div className={`flex items-start gap-2 p-3 rounded-lg text-sm ${result.success ? "bg-green-50 text-green-800 border border-green-200" : "bg-red-50 text-red-800 border border-red-200"}`}>
          ...
          <button onClick={() => setResult(null)} className="ml-auto text-inherit opacity-60 hover:opacity-100"><X size={13} /></button>
        </div>
      )}
      ...
      {loadingAccounts ? (
            ...
            <div key={acc.id} className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                {renamingId === acc.id ? (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onKeyDown={(e) => { if (e.key === "Enter") handleRename(acc.id); if (e.key === "Escape") setRenamingId(null); }}
                      className="border border-gray-300 rounded-lg px-2 py-1 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-brand-500"
                      autoFocus
                    />
                ...
                <div className="flex items-center gap-3 mt-0.5 flex-wrap">
                  ...
                  {acc.webhookRegistered && (
                    <span className="text-xs text-green-600 font-medium">● Webhook active</span>
                  )}
                  {acc.teamAccessCount > 0 && (
                    <span className="text-xs text-blue-600">{acc.teamAccessCount} member{acc.teamAccessCount > 1 ? "s" : ""} with access</span>
                  )}
                </div>
              </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
          <strong>No accounts yet.</strong> Import your first WhatsApp Cloud API credentials below to get started.
        </div>
      )}

      {/* Add account button / limit message */}
      {atLimit ? (
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 text-sm text-orange-800 flex items-center justify-between">
          <span>
            <strong>Account limit reached.</strong> Upgrade your plan to connect more WhatsApp accounts.
          </span>
          <a href="/pricing" className="ml-4 shrink-0 bg-orange-500 text-white text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-orange-600 transition-colors">
            Upgrade →
          </a>
        </div>
      ) : (
      ...
      {showForm && !atLimit && (
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <p className="text-sm text-gray-500 mb-5">
            ...
            </a>. All secrets are AES-256 encrypted before storing.
          </p>
              ...
              <div key={field.name}>
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={handleChange}
                    required
                    className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-500 pr-10"
                  />
                  <button
                    type="button"
